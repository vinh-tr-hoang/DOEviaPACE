...
def optimize(d_init: np.ndarray, ce_eit: CeEIT, total_iter: int,
             training_data_size: int, M: int) -> np.ndarray:
    """
    Optimize the experimental design parameters. It uses an iterative procedure where
    each iteration consists of two main steps: training an ANN to approximate
    the conditional expectation of the plies angles given the observational random variable,
     and then optimizing the design parameters using the trained ANN.

    Parameters:
    d_init (np.ndarray): Initial guess of the design parameters.
    ...
    for i in range(total_iter):
        ...
        # set the design-vector sampling distribution (normal, centered on the current design)
        ...
        # adaptively reduce the learning rate for the design optimization
        ...
        # only update the design when the ANN training converged well enough (loss < 1.02)
        if history.history['loss'][-1] < 1.02:
            # optimizing the design of experiment for 20 epochs
        ...
        # computing the total expected conditional variance. This is only for plotting purpose
...
def plot(ref_optimal_d: np.ndarray, overall_history: list, training_data_size: int):
    ...
    _epoch = _d.shape[0]  # Total number of iterations
...
if __name__ == '__main__':
    ...
    # These evaluations are used to train the ANN approximating the conditional mean.

    M = 500  # Total number of evaluations of the noise-free observational map used for optimizing the design vector.

    # An initial guess for the design parameters
    d_guess = np.array([1, 1, 1, 1, 1, -1, -1, -1, -1], dtype='float32') * 0.51

    # Run the optimization process to find the optimal design of experiments
    numerical_optimal_d, overall_history = optimize(d_guess, ce_eit, total_iter, training_data_size, M)
    ...
    plot(ref_optimal_d, overall_history, training_data_size)
